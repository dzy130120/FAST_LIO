...
Preprocess::Preprocess(ros::NodeHandle nh_)
  :feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1), nh(nh_)
{
  inf_bound = 10;
  N_SCANS   = 6;
  group_size = 8;
  ...
  pub_surf = nh.advertise<sensor_msgs::PointCloud2>("callback_surf", 1000);
}
...
void Preprocess::d455_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    pcl::PointCloud<D455_ros::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.points.size();
    pl_surf.reserve(plsize);
    ...
    
    {
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.y = pl_orig.points[i].y;
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = 0;//pl_orig.points[i].intensity;
        added_pt.curvature = pl_orig.points[i].t / 1000.0;
        ...
        if (i % point_filter_num == 0)
        {
          if(added_pt.x*added_pt.x+added_pt.y*added_pt.y+added_pt.z*added_pt.z > blind)
          {
            // ROS_INFO_STREAM("point x: "<<added_pt.x<<" y: "<<added_pt.y<<" z: "<<added_pt.z);
            pl_surf.points.push_back(added_pt);
            // printf("time mode: %d time: %d \n", given_offset_time, pl_orig.points[i].t);
          }
        }
...
void Preprocess::pub_func(PointCloudXYZI &pl, ros::Publisher& publisher, const ros::Time &ct)
{
  ...
  // ROS_INFO_STREAM("header: "<<output.header);
  // ROS_INFO_STREAM("fields: "<<output.fields[0]);
  output.header.frame_id = "d455";
